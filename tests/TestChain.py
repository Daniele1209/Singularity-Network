...
class MyChainTester(unittest.TestCase):
    ...
    def test_add_invalid_index_block(self):
        test_chain = self.chain_with_wallets()
        transactions = self.create_valid_transactions()
        block = self.test_forger.createBlock(0, transactions, test_chain._last_block.getHash)
    ...
    def test_add_invalid_signature_block(self):
        ...
        with self.assertRaises(BlockValidationError) as err:
            test_chain.push_block(block)
        self.assertEqual('Message: Invalid signature !, Arguments: {}', str(err.exception))
        self.reset_wallets()
        del test_chain
    ...
    def test_add_invalid_transaction_count_block(self):
        ...
        with self.assertRaises(BlockValidationError) as err:
            test_chain.push_block(block)
        self.assertEqual('Message: Transaction number exceeds the max !, Arguments: {}', str(err.exception))
        self.reset_wallets()
        del test_chain

    def test_add_valid_transaction(self):
        test_chain = self.chain_with_wallets()
        test_transaction = self.test_wallet_1.createTransaction(10, self.test_wallet_2.get_public_key(), 0.1 * 10)
    ...
    def test_add_invalid_balance_transaction(self):
        test_chain = self.chain_with_wallets()
        test_transaction = self.test_wallet_1.createTransaction(1000, self.test_wallet_2.get_public_key(), 0.1 * 10)
        with self.assertRaises(TransactionValidationError) as err:
            test_chain.transaction_validation(test_transaction)
        self.assertEqual('Message: Invalid balance !, Arguments: {}', str(err.exception))
        self.reset_wallets()
        del test_chain
    ...
    def test_add_invalid_signature_transaction(self):
        ...
        with self.assertRaises(TransactionValidationError) as err:
            test_chain.transaction_validation(test_transaction)
        self.assertEqual('Message: Transaction signature not valid !, Arguments: {}', str(err.exception))
        self.reset_wallets()
        del test_chain

    def test_add_invalid_low_amount_transaction(self):
        test_chain = self.chain_with_wallets()
        test_transaction = self.test_wallet_1.createTransaction(-5, self.test_wallet_2.get_public_key(), 0.1 * 10)
        with self.assertRaises(TransactionValidationError) as err:
            test_chain.transaction_validation(test_transaction)
        self.assertEqual('Message: Transaction amount too low !, Arguments: {}', str(err.exception))
        self.reset_wallets()
        del test_chain

    def test_add_invalid_low_fee_transaction(self):
        test_chain = self.chain_with_wallets()
        test_transaction = self.test_wallet_1.createTransaction(20, self.test_wallet_2.get_public_key(), 0.01)
        with self.assertRaises(TransactionValidationError) as err:
            test_chain.transaction_validation(test_transaction)
        self.assertEqual('Message: Fee lower than minimum !, Arguments: {}', str(err.exception))
        self.reset_wallets()
        del test_chain

    def test_add_invalid_same_accounts_transaction(self):
        test_chain = self.chain_with_wallets()
        test_transaction = self.test_wallet_1.createTransaction(20, self.test_wallet_1.get_public_key(), 0.1 * 20)
        with self.assertRaises(TransactionValidationError) as err:
            test_chain.transaction_validation(test_transaction)
        self.assertEqual('Message: Transaction payer is the same as payee !, Arguments: {}', str(err.exception))
        self.reset_wallets()
        del test_chain
    ...
    def test_remove_transactions(self):
        ...
        for t_idx in range(0, len(out_transactions)):
            self.assertEqual(out_transactions[t_idx].toJson(), transactions_to_check[t_idx].toJson())
        self.reset_wallets()
        del test_chain
    ...
    def test_process_block(self):
        ...
        test_transaction = Transaction(6, genesis_dev_address, block.get_forger(), fee=minimum_fee, type='REWARD')
        self.assertEqual(test_transaction.toJson(), test_chain.pendingTransactions[-1].toJson())
        self.reset_wallets()
        del test_chain
    ...
    def test_execute_transactions(self):
        ...
        balance_acc1 = test_chain.account_model.get_balance(self.test_wallet_1.get_public_key())
        balance_acc2 = test_chain.account_model.get_balance(self.test_wallet_2.get_public_key())
    ...
    def test_invalid_balance_account_model(self):
        ...
        self.assertEqual('Message: Account not found -get !, Arguments: {}', str(err.exception))
        del test_chain

    def test_invalid_update_account_model(self):
        test_chain = Chain()
        with self.assertRaises(AccountModelError) as err:
            test_chain.account_model.balance_update(self.test_wallet_1.get_public_key(), 420)
        self.assertEqual('Message: Account not found -update !, Arguments: {}', str(err.exception))
        del test_chain
